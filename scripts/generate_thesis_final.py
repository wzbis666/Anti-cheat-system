...
def add_figure_placeholder(doc, fig_num, fig_title):
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run(f'【图{fig_num} {fig_title}】')
    set_run_font(run, 'SimHei', 10.5, bold=True)
...
def create_thesis():
    ...
    add_paragraph(doc, '''随着网络游戏的快速发展，游戏作弊行为日益猖獗，严重影响了游戏的公平性和用户体验。Minecraft作为全球最受欢迎的沙盒游戏之一，其服务器面临着各种作弊行为的威胁，传统的反作弊方案存在检测效率低、误报率高、管理不便等问题。''')
    
    add_paragraph(doc, '''本文采用Spring Boot 3.2框架作为后端开发平台，结合Vue 3前端框架、MySQL数据库和WebSocket实时通信技术，设计并实现了一个完整的Minecraft服务器反作弊管理系统。论文主要完成了系统需求分析、架构设计、数据库设计、功能模块开发及系统测试等工作，实现了用户管理、玩家管理、作弊检测与记录、封禁管理、举报处理、白名单管理和数据统计等功能模块。''')
    
    add_paragraph(doc, '''相比其他同类系统，本系统具有以下特色：采用前后端分离架构，提高了系统的可维护性和可扩展性；通过WebSocket实现作弊行为的实时告警，响应速度快；支持多种作弊类型的检测，包括飞行作弊、速度作弊、杀戮光环等；提供直观的数据可视化界面，方便管理员监控和分析。''')
    ...
    add_paragraph(doc, '''在软件开发与项目实施过程中，可行性研究是保障项目顺利推进的关键环节，通过对系统多维度条件进行综合论证，可提前识别潜在风险、优化实施方案，为系统研发与落地提供科学依据。本次Minecraft服务器反作弊管理系统从经济可行性、技术可行性、操作可行性、社会可行性、法律可行性五个方面开展论证。''')
    ...
    add_heading(doc, '3.2.1 系统功能需求', 3)
    add_paragraph(doc, '''系统应具备高效的响应性能，保障用户各类操作均可快速反馈，对数据库查询等核心业务进行深度优化以缩短处理时长；可支撑大量用户并发在线访问，确保业务高峰时段系统运行稳定、无卡顿；采用图片懒加载等优化策略提升页面渲染与加载效率，优化用户交互体验；同时系统架构具备良好的可扩展性，支持后续功能模块的灵活扩展。''')
    add_paragraph(doc, '''系统应具备高可靠性，可长期稳定运行并保持极高的服务可用水平。系统配备完善的异常处理机制，能够自动捕获并妥善处理各类运行异常，有效规避系统崩溃等严重问题。数据库执行常态化备份策略，全方位保障业务数据的完整性与安全性。系统具备良好的容错能力，当局部功能发生故障时，不会影响其余功能模块的正常运行。同时系统提供完整的日志记录功能，全程留存系统运行状态与用户操作行为，为后续故障排查、运维优化提供数据支撑。''')
    add_paragraph(doc, '''系统界面应简洁美观，符合用户使用习惯。操作流程应简单明了，用户无需培训即可快速上手。系统应提供友好的错误提示信息，帮助用户理解和解决问题。系统应支持多种设备和屏幕尺寸，确保在不同设备上都能良好显示。系统应提供在线帮助和操作指南，方便用户随时查阅。系统应支持中英文切换，满足不同用户的语言需求。''')
    
    add_heading(doc, '3.3 系统功能模块分析', 2)
    add_paragraph(doc, '''根据需求分析，系统功能划分为八个主要模块：用户管理模块、玩家管理模块、作弊检测模块、封禁管理模块、举报管理模块、白名单管理模块、数据统计模块和系统设置模块。系统功能模块层次图如图3-1所示，各模块之间相互独立又协同工作，共同构成完整的反作弊管理系统。''')
    
    add_figure_placeholder(doc, '3-1', '系统功能模块层次图')
    ...
    add_paragraph(doc, '''用例图是系统的蓝图，呈现了参与者、用例以及它们之间的关系。本系统的参与者包括管理员、普通用户和MC玩家三种角色。管理员可以执行用户登录、玩家管理、作弊记录管理、封禁管理、举报处理、白名单管理、数据统计和系统设置等用例。普通用户可以执行用户登录、用户注册、个人信息管理和密码修改等用例。MC玩家可以执行游戏内举报和查看封禁状态等用例。系统用例图如图3-2所示。''')
    
    add_figure_placeholder(doc, '3-2', '系统用例图')
    ...
    add_table(doc, usecase_headers, usecase_rows, '表3-1 用例表')
    
    add_heading(doc, '3.4 数据流图', 2)
    add_paragraph(doc, '''数据流图是一种图形化工具，用于描述系统中数据的流动和处理过程。本系统采用分层的数据流图设计方法，包括顶层数据流图和0层数据流图，清晰展示系统与外部实体之间的数据交互关系。''')
    add_paragraph(doc, '''本系统的顶层数据流图如图3-3所示，展示了系统与外部实体之间的数据交互关系。外部实体包括管理员、MC玩家和游戏服务器，系统主要处理过程包括处理用户请求、作弊数据处理、封禁管理处理、举报处理和统计分析处理，数据存储包括用户数据库、玩家数据库、作弊记录库、封禁记录库和举报记录库。''')
    
    add_figure_placeholder(doc, '3-3', '系统顶层数据流图')
    
    add_paragraph(doc, '''0层数据流图将顶层处理分解为多个主要功能模块，展示各模块之间的数据流动关系。本系统包括5个主要处理模块和5个数据存储，如图3-4所示。''')
    
    add_figure_placeholder(doc, '3-4', '系统0层数据流图')
    ...
    add_paragraph(doc, '''系统整体架构分为用户层、展示层、网关层、业务层和数据层五个层次。用户层包括管理员、普通用户、MC玩家和系统管理员四种角色。展示层包括Vue 3前端应用和Minecraft服务器两部分。网关层采用Spring Security Filter Chain实现安全过滤。业务层基于Spring Boot应用实现。数据层采用Spring Data JPA进行数据访问，底层使用MySQL数据库。系统架构图如图4-1所示。''')
    
    add_figure_placeholder(doc, '4-1', '系统架构图')
    
    add_heading(doc, '4.1.2 系统功能流程设计', 3)
    add_paragraph(doc, '''系统功能流程图描述了用户使用系统的整体流程。如图4-2所示，用户首先访问系统，判断是否已登录，未登录则显示登录页面，输入用户名密码进行验证，验证通过后生成JWT Token并跳转主页。已登录用户可以选择不同的功能模块进行操作，包括玩家管理、作弊记录、封禁管理、举报管理、白名单、数据统计和系统设置等。操作完成后可选择继续操作或退出登录。''')
    
    add_figure_placeholder(doc, '4-2', '系统功能流程图')
    
    add_heading(doc, '4.2 详细设计', 2)
    add_paragraph(doc, '''详细设计阶段需要画出每个功能模块的业务流程图，并在正文中有详细的文字说明。''')
    
    add_heading(doc, '4.2.1 作弊检测业务流程', 3)
    add_paragraph(doc, '''作弊检测业务流程图如图4-3所示。玩家执行游戏行为后，插件监听玩家事件，检测是否作弊。如果检测到作弊，则收集作弊数据，通过WebSocket发送到后端。后端接收数据后，保存作弊记录，更新玩家风险评分。如果风险评分达到阈值，则触发自动封禁，创建封禁记录并踢出玩家；否则向前端推送告警，管理员查看告警后可选择是否手动封禁。''')
    
    add_figure_placeholder(doc, '4-3', '作弊检测业务流程图')
    
    add_heading(doc, '4.2.2 举报处理业务流程', 3)
    add_paragraph(doc, '''举报处理业务流程图如图4-4所示。玩家在游戏中使用/report命令，输入被举报玩家和原因。插件发送举报到后端，后端保存举报记录，举报进入待处理状态。管理员查看举报列表，选择待处理举报，查看举报详情后进行审核。审核结果分为确认违规和驳回举报两种，确认违规后可选择是否封禁玩家，最后记录处理结果并通知举报人。''')
    
    add_figure_placeholder(doc, '4-4', '举报处理业务流程图')
    
    add_heading(doc, '4.3 数据库设计', 2)
    
    add_heading(doc, '4.3.1 概念模型', 3)
    add_paragraph(doc, '''数据库概念设计是对现实实体及关联关系的抽象模型化，可帮助研发团队清晰理解系统需求、数据对象与关联特性，为后续系统设计与实现奠定坚实基础。在数据库系统中，常使用E-R图表示实体间的联系与属性。E-R图可帮助设计师可视化呈现数据要素的关联，明确数据结构与流向，精准匹配系统需求。''')
    add_paragraph(doc, '''系统总体E-R图如图4-5所示，展示了玩家实体与作弊记录、封禁记录、举报记录、白名单实体之间的关系。一个玩家可以产生多条作弊记录，接受多次封禁，被多次举报，也可以被加入白名单。''')
    
    add_figure_placeholder(doc, '4-5', '系统总体E-R图')
    
    add_paragraph(doc, '''各实体的E-R图分别如图4-6至图4-11所示，展示了每个实体的所有属性。''')
    
    add_figure_placeholder(doc, '4-6', '玩家实体E-R图')
    add_figure_placeholder(doc, '4-7', '作弊记录实体E-R图')
    add_figure_placeholder(doc, '4-8', '封禁记录实体E-R图')
    add_figure_placeholder(doc, '4-9', '举报记录实体E-R图')
    add_figure_placeholder(doc, '4-10', '管理员实体E-R图')
    add_figure_placeholder(doc, '4-11', '白名单实体E-R图')
    ...
    add_figure_placeholder(doc, '5-1', '用户登录界面')
    
    add_paragraph(doc, '''用户登录功能的核心代码如下所示。系统接收用户名和密码参数，调用AdminService验证登录信息，验证通过后生成JWT Token，构建用户信息对象返回给前端。''')
    
    add_figure_placeholder(doc, '5-2', '用户登录核心代码')
    
    add_heading(doc, '5.3 玩家管理模块实现', 2)
    add_paragraph(doc, '''玩家管理模块负责管理被监控的Minecraft玩家信息。玩家列表界面如图5-3所示，展示所有玩家的名称、UUID、风险评分、最后在线时间等信息。支持按玩家名称搜索，按风险等级筛选，支持修改风险评分和删除玩家等操作。''')
    
    add_figure_placeholder(doc, '5-3', '玩家管理界面')
    ...
    add_paragraph(doc, '''作弊检测模块是系统的核心模块，负责接收和处理作弊数据。仪表盘界面如图5-4所示，展示系统概览统计和实时告警列表。当游戏服务器检测到作弊行为时，通过WebSocket将数据发送到后端，后端保存记录后向前端推送告警，管理员可以实时查看作弊信息。''')
    
    add_figure_placeholder(doc, '5-4', '仪表盘界面')
    
    add_paragraph(doc, '''WebSocket处理器核心代码如下所示。处理器接收游戏服务器发送的JSON格式作弊数据，解析后保存到数据库，更新玩家风险评分，并向所有连接的前端广播告警信息。''')
    
    add_figure_placeholder(doc, '5-5', 'WebSocket处理器核心代码')
    
    add_heading(doc, '5.5 封禁管理模块实现', 2)
    add_paragraph(doc, '''封禁管理模块负责对作弊玩家进行处罚。封禁管理界面如图5-6所示，展示所有封禁记录，包括玩家名称、封禁类型、封禁时间、封禁原因等信息。支持封禁玩家、解封玩家操作，支持按封禁状态筛选。''')
    
    add_figure_placeholder(doc, '5-6', '封禁管理界面')
    
    add_paragraph(doc, '''封禁功能支持永久封禁和临时封禁两种类型。永久封禁没有期限限制，临时封禁需要设置封禁时长。系统通过定时任务自动检查并解除到期的临时封禁。''')
    
    add_heading(doc, '5.6 举报管理模块实现', 2)
    add_paragraph(doc, '''举报管理模块负责处理玩家举报。举报管理界面如图5-7所示，展示所有举报记录，包括举报人、被举报人、举报原因、处理状态等信息。管理员可以审核举报，选择确认违规或驳回举报，确认违规后可选择同时封禁玩家。''')
    
    add_figure_placeholder(doc, '5-7', '举报管理界面')
    
    add_paragraph(doc, '''游戏内举报功能通过/report命令实现，玩家输入被举报玩家名称和举报原因后，插件将举报信息发送到后端保存。''')
    
    add_heading(doc, '5.7 白名单模块实现', 2)
    add_paragraph(doc, '''白名单模块负责管理白名单玩家。白名单界面如图5-8所示，展示所有白名单玩家，支持添加白名单、移除白名单操作。白名单玩家可以跳过部分检测规则，适用于服务器管理员信任的玩家。''')
    
    add_figure_placeholder(doc, '5-8', '白名单管理界面')
    ...
    add_figure_placeholder(doc, '5-9', '数据统计图表')
    ...
    add_paragraph(doc, '''时光荏苒，四年的大学生活即将画上句号。在完成本篇毕业论文之际，我要向所有给予我帮助和支持的人表达最诚挚的感谢。''')
    add_paragraph(doc, '''首先，我要感谢我的指导老师。在论文选题、开题、撰写和修改的整个过程中，老师给予了我悉心的指导和耐心的帮助。老师严谨的治学态度、渊博的专业知识和认真负责的工作精神，使我受益匪浅。老师的每一次指导都让我对研究内容有了更深入的理解，每一次修改建议都让论文更加完善。''')
    add_paragraph(doc, '''其次，我要感谢我的任课老师们。四年来，老师们传授的专业知识为我完成毕业设计奠定了坚实的基础。从程序设计基础到数据结构，从计算机网络到软件工程，每一门课程都让我收获了宝贵的知识和技能。''')
    add_paragraph(doc, '''感谢我的同学们。在学习和生活中，同学们给予了我很多帮助和鼓励。特别是在毕业设计过程中，大家互相交流、互相帮助，共同解决遇到的问题，让我感受到了集体的温暖。''')
    add_paragraph(doc, '''感谢我的家人。感谢父母多年来的养育之恩和无私奉献，是你们的支持让我能够专心学业；感谢你们的理解和包容，让我能够自由地追求自己的梦想。''')
    add_paragraph(doc, '''最后，感谢所有参考文献的作者们。你们的研究成果为我的毕业设计提供了重要的参考和借鉴。''')
    add_paragraph(doc, '''毕业不是终点，而是新的起点。我将带着在大学期间学到的知识和技能，带着老师和同学们的祝福，勇敢地迎接未来的挑战。''')
    
    doc.save('docs/figures/本科毕业论文_最终版.docx')
    print('论文已生成: docs/figures/本科毕业论文_最终版.docx')
